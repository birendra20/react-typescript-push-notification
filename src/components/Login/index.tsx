...
import {
  ...
  goToSignupStyle,
  loginBtnStyle,
  loginSampleUsersContainerStyle,
  loginStyle,
  loginUidFormStyle,
  noAccountStyle,
  userAvatarStyle,
  userBtnStyle,
  userNameAndUidContainerStyle,
  userNameStyle,
  userUidStyle,
...
export function Login(props: ILoginProps) {
  ...
  function getUserBtnWithKeyAdded({ name, uid, avatar }: User) {
    return (
      <button key={uid} onClick={() => login(uid)} style={userBtnStyle(theme)}>
        <img src={avatar} alt={`${name}'s avatar`} style={userAvatarStyle()} />
        <span style={userNameAndUidContainerStyle()}>
          <span style={userNameStyle(theme)}>{name}</span>
          <span style={userUidStyle(theme)}>{uid}</span>
        </span>
      </button>
    );
  }
  ...
  return (
    <LoginSignup title='Login to your account'>
      <div style={loginStyle()}>
        <div style={loginSampleUsersContainerStyle()}>
          <div style={usingSampleUsersTextStyle(theme)}>
